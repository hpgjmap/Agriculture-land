...
}`),a.uniforms.add(new hi("stipplePatternTexture",s=>s.stippleTexture),new Y("stipplePatternSDFNormalizer",s=>Tr(s.stipplePattern)),new Y("stipplePatternPixelSizeInv",s=>1/$t(s))),e.stippleOffColorEnabled&&a.uniforms.add(new Be("stippleOffColor",s=>vr(s.stippleOffColor))),a.code.add(p`float getStippleSDF(out bool isClamped) {
...
return isClamped ? floor(antiAliasedResult + 0.5) : antiAliasedResult;
  ...
  `)}function Tr(t){return t?(Math.floor(.5*(mr(t)-1))+.5)/t.pixelRatio:1}function $t(t){const e=t.stipplePattern;return e?fr(t.stipplePattern)/e.pixelRatio:1}const _r=.4,wt=64,br=wt/2,Or=br/5,yr=wt/Or,ya=.25;function Er(t,e){const i=t.vertex;qe(i),i.uniforms.get("markerScale")==null&&i.constants.add("markerScale","float",1),i.constants.add("markerSizePerLineWidth","float",yr).code.add(p`float getLineWidth() {
